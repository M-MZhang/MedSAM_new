...
modality = "CT"
anatomy = "5"  # anantomy + dataset name
img_name_suffix = "_0000.nii"
# nii_suffix = "_0000.nii"
gt_name_suffix = ".nii"
prefix = modality + "_" + anatomy + "_"

nii_path = "../medsam-data/output/high"  # path to the nii images
gt_path = "../medsam-data/output/mask"  # path to the ground truth
npy_path = "../medsam-data/npy/" + prefix[:-1]
...
# for bones
WINDOW_LEVEL = 400  # only for CT images
WINDOW_WIDTH = 1800  # only for CT images
...
for name in tqdm(names[:40]):  # use the remaining 10 cases for validation
    ...
    gt_name = name
    gt_sitk = sitk.ReadImage(join(gt_path, gt_name)) #[z,y,x]
    ...
    if len(z_index) > 0:
        ...
        sitk.WriteImage(
            img_roi_sitk,
            join(npy_path, gt_name.split(gt_name_suffix)[0] + "_img.nii.gz"),
        )
        sitk.WriteImage(
            gt_roi_sitk,
            join(npy_path, gt_name.split(gt_name_suffix)[0] + "_gt.nii.gz"),
        )
        # save the each CT image as npy file
        for i in range(img_roi.shape[0]):
            img_i = img_roi[i, :, :] #[88, 128]
            img_3c = np.repeat(img_i[:, :, None], 3, axis=-1) #[88, 128, 3]
            ...
            # plt.imshow(resize_img_skimg_01)
            # plt.show()
            np.save(
                join(
                    npy_path,
